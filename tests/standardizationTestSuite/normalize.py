...
# Begin JS
    Normalization('Diazo/azo to C=N+=N-', '[C-:1][N+:2]#[N:3]>>[C+0:1]=[N+:2]=[N-:3]'),
    Normalization('Phosphine ', '[PH3+:1]>>[PH2+0:1]'),
    Normalization('Nitro from one OH ', '[N+:1](=[O:2])([OH:3])>>[N+1:1]([O-1:2])=[O:3]'),
    Normalization('Nitro from two OH ', '[N:1]([OH:2])([OH:3])>>[N+1:1]([O-1:2])=[O:3]'),
# End JS
...
class Normalizer(object):
    ...
    def _normalize_fragment(self, mol):
        for n in six.moves.range(self.max_restarts):
            ...
            for normalization in self.normalizations:
                #log.debug("Normailzation %s", str(normalization.transform))
    ...
    def _apply_transform(self, mol, rule):
        ...
        """
        mols = [mol]
        #log.debug("Input smiles %s", str(Chem.MolToSmiles(mol)))
        for n in six.moves.range(20):
            ...
            for mol in mols:
                #log.debug("Here 2")
                #log.debug(str(rule))
                #log.debug(str(rule.RunReactants((mol,))))
                for product in [x[0] for x in rule.RunReactants((mol,))]:
                    #log.debug("Here 3")
                    #log.debug("Considering product %s", str(product))
                    if Chem.SanitizeMol(product, catchErrors=True) == 0:
                        #print "The product is sane"
                        #print "Output smiles ", Chem.MolToSmiles(product)
