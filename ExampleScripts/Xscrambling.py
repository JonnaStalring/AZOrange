...
def addXscramVar(train, nXscram):

    import random
    print "Number of attr before Xscramble ", len(train.domain.attributes)
    attrList = train.domain.attributes
    nAttr = len(attrList)
    nEx = len(train)
    for idx in range(nXscram):
        attrList.append(orange.FloatVariable("RandVar"+str(idx)))        
    newDomain = Orange.data.Domain(attrList, train.domain.classVar)
    newData = Orange.data.Table(newDomain, train)
    for ex in newData:
        for idx in range(nXscram):
            # "Rand selection of an element in the descriptor matrix"
            rowIdx = random.randint(0,nEx-1) 
            colIdx = random.randint(0, nAttr-1)
            #print train[rowIdx][colIdx].value
            ex["RandVar"+str(idx)] = train[rowIdx][colIdx].value
    print "Number of attr after Xscramble ", len(newData.domain.attributes)
    return newData
...
def getAccStat(rankSumTuple, nDesc, train, randTest, extTest, resultsFid, projectName):
    ...
    print "Before selection ",  len(train.domain.attributes)
    attrList = []
    for elem in rankSumTuple:
        if len(attrList) < (nDesc) and elem[0] not in attrList:
            attrList.append(elem[0])
    train = dataUtilities.attributeSelectionData(train, attrList)
    randTest = dataUtilities.attributeSelectionData(randTest, attrList)
    extTest = dataUtilities.attributeSelectionData(extTest, attrList)
    ...
    for attr in train.domain.attributes:
        print attr.name

    print "Xscrambling... "
    nXscram = 5
    train = addXscramVar(train, nXscram)
    randTest = addXscramVar(randTest, nXscram)
    extTest = addXscramVar(extTest, nXscram)
...
def Wrapper(train, randTest, extTest, resultsFid, projectName, MCCdict, descList, MCCfile):
...
#    tuplePrint(rankSumTuple, 50)
    ...
    for idx in range(100):
        ...
        MCC_CVList.append(MCC_CV)
        MCC_randList.append(MCC_rand)
        MCC_extList.append(MCC_ext)
   
        fid = open(MCCfile, "a")
        fid.write(str(MCC_CV)+"\t"+str(MCC_rand)+"\t"+str(MCC_ext)+"\n") 
        fid.close() 
...
if __name__ == "__main__":
    ...
    descList = [20]
    resultsFile = "accResultsXscramble.txt"
    descFile = "descSelectionResultsXscramble.txt"
    MCCfile = "MCC_Xscramble20_malt.txt"

    MCCfid = open(MCCfile, "w")
    MCCfid.write("MCC_CV\tMCC_rand\tMCC_ext\n")
    MCCfid.close()
    ...
    for projectName in projectList:
        ...
        MCCdict[projectName] = {}
        MCCdict = Wrapper(train, randTest, extTest, resultsFid, projectName, MCCdict, descList, MCCfile)
        resultsFid.close()
